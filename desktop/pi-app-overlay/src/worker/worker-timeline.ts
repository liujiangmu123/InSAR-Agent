...
import { mergeImagesIntoDetails } from '../extension-compat/json-path.js'
  ...
  for (const m of messages) {
    ...
    if (pm.role === 'user') {
      ...
      if (targetIdx !== undefined && items[targetIdx]) {
        ...
        const details = mergeImagesIntoDetails(pm.details, pm)
        if (details !== undefined) items[targetIdx].toolDetails = details
        ...
        items.push({
          ...
          ...(details !== undefined ? { toolDetails: details } : {}),
  ...
  for (const entry of path) {
    ...
    if (m.role === 'user') {
      ...
      if (targetIdx !== undefined && items[targetIdx]) {
        items[targetIdx].toolOutput = text.slice(0, 4000)
        if (callId) items[targetIdx].toolCallId = callId
        const details = mergeImagesIntoDetails(m.details, m)
        if (details !== undefined) items[targetIdx].toolDetails = details
