...
interface Asset { path?: string; url?: string; name?: string; label?: string; dataUrl?: string }
...
function collectAssets(details: Record<string, unknown> | null | undefined, out: string): Asset[] {
  ...
  const push = (a: Asset) => {
    const key = a.path || a.url || a.dataUrl || ''
    if (!key || seen.has(key)) return
    seen.add(key)
    assets.push(a)
  }
  const d = details as { file?: string; url?: string; paths?: unknown[]; images?: Array<{ path?: string; url?: string; name?: string; dataUrl?: string }> } | null | undefined
  ...
  if (Array.isArray(d?.images)) {
    for (const img of d.images) {
      push({ path: img.path, url: img.url, name: img.name, dataUrl: img.dataUrl })
    }
  }
...
function InlineImage({ path, dataUrl, workspaceRoot, enabled }: { path?: string; dataUrl?: string; workspaceRoot: string | null; enabled: boolean }) {
  const [src, setSrc] = useState<string | null>(dataUrl || null)
  const [err, setErr] = useState(false)
  useEffect(() => {
    if (!enabled) { setSrc(null); setErr(false); return }
    if (dataUrl) { setSrc(dataUrl); setErr(false); return }
    if (!path || !workspaceRoot) { setSrc(null); setErr(false); return }
    let cancelled = false
  ...
  }, [path, dataUrl, workspaceRoot, enabled])
...
const MediaTemplate: ToolCardComponent = ({ item }) => {
  ...
  const preview = assets.find((a) => a.dataUrl || a.path)

  return (
    <div className="mt-1 space-y-2 rounded-md border border-border/35 p-2">
      {preview && <InlineImage path={preview.path} dataUrl={preview.dataUrl} workspaceRoot={workspace} enabled={showInline} />}
