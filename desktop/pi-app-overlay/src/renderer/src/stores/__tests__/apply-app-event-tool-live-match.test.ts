...
describe('live tool match by toolCallId only', () => {
  ...
  it('merges image content blocks from the tool result envelope into details', () => {
    const timelineItems: TimelineItem[] = [
      {
        id: 't1',
        type: 'tool-call',
    ...
    handleTool(
      {
        ...
        details: { name: 'velocity.png', step: 10 },
        isError: false,
        runId: 'r1',
        timestamp: 2,
        sessionId: 's',
    ...
    expect(timelineItems[0].toolOutput).toBe('step 10 · velocity.png')
    expect(timelineItems[0].toolDetails).toMatchObject({
      name: 'velocity.png',
      step: 10,
