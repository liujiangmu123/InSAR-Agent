...
export type OpenLocalPath = (absPath: string) => Promise<string>

async function defaultOpenLocalPath(absPath: string): Promise<string> {
  const { shell } = await import('electron')
  return shell.openPath(absPath)
}

let openLocalPathImpl: OpenLocalPath = defaultOpenLocalPath

/** Test seam: inject Explorer opener without loading Electron. */
export function setOpenLocalPathForTests(fn: OpenLocalPath | null): void {
  openLocalPathImpl = fn ?? defaultOpenLocalPath
}

function actionView(adapterId: string, cwd: string): Record<string, unknown> {
  const local = readAdapterConfig(adapterId, cwd)
  const raw = readRawView(adapterId)
  const homeDir =
    (typeof process.env.INSAR_HOME === 'string' && process.env.INSAR_HOME.trim()) ||
    (typeof local.homeDir === 'string' && local.homeDir.trim()) ||
    'workspace'
  const exportsDir =
    (typeof process.env.INSAR_EXPORT_DIR === 'string' && process.env.INSAR_EXPORT_DIR.trim()) ||
    (typeof local.exportsDir === 'string' && local.exportsDir.trim()) ||
    'pi-insar/exports'
  const apiBaseHint =
    (typeof local.apiBaseHint === 'string' && local.apiBaseHint.trim()) ||
    (typeof raw.apiBaseHint === 'string' && String(raw.apiBaseHint).trim()) ||
    (typeof process.env.INSAR_API_BASE === 'string' && process.env.INSAR_API_BASE.trim()) ||
    'http://127.0.0.1:8873'
  return {
    ...raw,
    ...local,
...
export async function runAdapterAction(
  ...
  if (action.type === 'openPath') {
    const view = actionView(adapterId, cwd)
    const target = tpl(action.url || '', view)
    const resolved = resolveAdapterOpenPath(target, cwd)
    if (!resolved.ok) return { ok: false, error: resolved.error }
    try {
      const err = await openLocalPathImpl(resolved.abs)
      if (err) return { ok: false, error: err, lines: [resolved.abs] }
      return { ok: true, lines: [resolved.abs] }
    } catch (e: unknown) {
      return { ok: false, error: e instanceof Error ? e.message : String(e), lines: [resolved.abs] }
    }
  }
