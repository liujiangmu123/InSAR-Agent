  ...
  for (const a of adapterPanels) {
    ...
    if (existing) {
      byId.set(a.panelId, {
        ...
        defaultActive: a.defaultActive ?? existing.defaultActive,
      })
      continue
    }
    byId.set(a.panelId, {
...
function catalogIdsWithDefaultActiveFirst(catalog: RightPanelCatalogItem[]): string[] {
  const ids = catalogPanelIds(catalog)
  const active = ids.filter((id) => catalog.find((c) => c.id === id)?.defaultActive)
  const rest = ids.filter((id) => !active.includes(id))
  return [...active, ...rest]
}

function hasStoredKnownOrder(raw: unknown, known: Set<string>): boolean {
  if (!Array.isArray(raw) || raw.length === 0) return false
  return raw.some((x) => typeof x === 'string' && known.has(x))
}

/** 与 catalog 对齐的 Tab 顺序；无已存顺序时 defaultActive 面板排最前；未知 id 追加在末尾 */
export function normalizeRightPanelOrder(raw: unknown, catalog: RightPanelCatalogItem[]): string[] {
  const ids = catalogPanelIds(catalog)
  const known = new Set(ids)
  if (!hasStoredKnownOrder(raw, known)) {
    return catalogIdsWithDefaultActiveFirst(catalog)
  }
  const out: string[] = []
...
export function firstEnabledPanel(
  ...
  const seq = normalizeRightPanelOrder(order, catalog)
