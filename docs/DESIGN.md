...
**明确不做**：生成命令行、写代码、开放式自由规划（任意命令/任意工具序列）、碰数据矩阵。受约束的自主循环（动作白名单闭集 + 周期上限 + 熔断 + 审批门内置，见 `docs/AGENT-LOOP.md`）是支持的，与本条不冲突（2026-08-14 波次修订，原表述「自由规划多步」）。
...
3. **单周期单决策，回合可多周期**（2026-08-14 波次修订，原「绝不让模型串多步——每次只问一个决策」）：每周期仍只做一个候选集内选择或一个白名单动作，周期间由确定性循环驱动器衔接；周期上限、同签名熔断、预算照 `AGENT-DESIGN.md §3.3` 与 `docs/AGENT-LOOP.md` 执行
   原依据（社区 issue 记录 Ollama 7b/14b/32b 一律「第二个工具永不执行」）保留为「本地小模型默认关闭自主循环」的开关条件
