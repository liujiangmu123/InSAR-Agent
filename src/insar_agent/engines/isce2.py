...
def _build_stripmap(*, cap: Capability, params: dict[str, Any],
    ...
    script = (
        "#!/usr/bin/env bash\nset -euo pipefail\ncd isce2\n"
        f"stripmapApp.py {Path(xml_rel).name} --start={start} --end={end}\n"
    )
...
def build(*, cap: Capability, method: str, params: dict[str, Any], run: dict,
    ...
    if "stripmap" in method:
        return _build_stripmap(cap=cap, params=params, workspace=workspace)
