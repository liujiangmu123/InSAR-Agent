...
description: ALOS 条带(stripmap)同震干涉场景:2010 El Mayor–Cucapah(Baja)Mw 7.2 地震 ALOS raw 条带对,ISCE2 stripmapApp 全链 WSL 实测通过(docs/VALIDATION-isce2-wsl.md)。3-6 步(配准/干涉/滤波/解缠)覆写为 isce2_stripmap_* 方法,单干涉对、无云端已完成步骤。适配「ALOS/条带/stripmap/L波段」类请求。
metadata:
  version: 1.0.0
  priority: 5
  label: 条带同震干涉
...
- 工作区布局按 registry 第 3 步参数默认值:`data/raw/reference/IMG-HH*` 与 `LED*`、
  `data/raw/secondary/IMG-HH*` 与 `LED*`;更换数据时只覆写第 3 步路径参数
  (science 输入,进指纹),4-6 步分段续跑同一份 stripmapApp 配置。
- 数据待装配(data_ready=false):GMTSAR 官方示例包 ALOS Baja EQ 本地已有,
  但需按上述布局装配 raw/LED 并完成 DEM 转换后才可开跑。
...
路径改为绝对路径;放到第 3 步 `dem_path` 参数指向的 `data/dem/dem.wgs84`。
第 2 步选 `dem_local`(本地 DEM),不走在线 DEM 服务。
