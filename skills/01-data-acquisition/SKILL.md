...
- **`asf_search_slc`**:需要原始 SLC 且要对第 2–6 步保留完整控制权时选它——自定多视比、滤波强度、解缠代价函数,或后续要走 PS(Persistent Scatterer,永久散射体)链(PS 必须从 SLC 起算,HyP3 成品进不了 PS 链)。代价:磁盘 `scenes * 2.4` GB、下载耗时(total 超时 6 h)、本机需具备 2–6 步引擎。
...
- **`scenes`(2–200,默认 7)**:
  ...
  - 上限约束:磁盘预检按 `scenes * 2.4` GB 估算,200 景 ≈ 480 GB,先看盘再填数。
...
5. **症状**:磁盘预算预检直接拒绝执行。**根因**:`scenes * 2.4` GB 超出可用空间(峰值另乘系数)。**处置**:减 `scenes`、对旧 run 产物做 GC、或改 HyP3 路线(Ridgecrest 实测 11 对产品仅 402 MB)。
...
- 尺寸合理性:S1 IW SLC 单景 GB 量级(预算按 2.4 GB/景);离群小文件按损坏处理,不要带病进下游。
