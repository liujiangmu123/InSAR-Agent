/* ============================================================
   resultsdraft 的无浏览器自查脚本(node prototype/resultsdraft.check.mjs)
   用最小 DOM stub 直接 import js/resultsdraft.js,断言:
   ① 纯函数:请求体组装 / 徽章文案与配色映射(是=蓝、否回退骨架=绿)/
     错误文案(404 无 run / 0 不可达 / 其他状态码);
   ② 自初始化挂载:#pane-report 出现 → 区块挂 pane 尾部且位于方法草稿模块
     (.reportdraft)之后,「生成结果章节」按钮在场;幂等重复初始化;
   ③ 生成流程:点击 → POST /api/report/results(body 带 session)→
     渲染草稿(.shell 等宽预格式)+ 「LLM 润色:是」徽章 + 落盘注明;
   ④ 一键复制:点「复制全文」→ 剪贴板收到草稿全文 + toast 复制文案;
   ⑤ 骨架回退:llm_polish=false → 徽章「LLM 润色:否(回退骨架)」;
   ⑥ 失败语义:404 → 「还没有可生成结果章节的 run」提示,且不清掉已有结果;
   ⑦ dock 重渲染 → 观察器自动重挂,已生成结果不丢失。
   只依赖 node 内建能力,零 npm 依赖(check() 约定与其余 check.mjs 一致)。
   ============================================================ */

/* ---------------- 最小 DOM stub(reportdraft.check.mjs 同款) ---------------- */
class NodeBase {
...
/* ---------------- 被测模块 ---------------- */
const RD = await import('./js/resultsdraft.js');
...
check('错误文案:404 = 还没有可生成结果章节的 run',
  RD.errorText(404).includes('还没有可生成结果章节的 run'));
...
/* 预置方法草稿区块占位:断言本区块挂在它之后(index.html 脚本顺序的模拟) */
const methodsStub = new Element('section');
methodsStub.className = 'reportdraft';
pane.appendChild(methodsStub);
dockBody.appendChild(pane);
DOC.body.appendChild(dockBody);
RD.initResultsDraft();
await sleep(10);
const mounted = () => pane.querySelectorAll('.resultsdraft');
check('区块挂进报告 pane 尾部', mounted().length === 1);
check('区块位于方法草稿模块(.reportdraft)之后',
  pane.children.indexOf(mounted()[0]) > pane.children.indexOf(methodsStub));
const genBtn = () => pane.querySelectorAll('button')
  .find((b) => b.textContent.includes('生成结果章节') || b.textContent.includes('生成中'));
check('「生成结果章节」按钮在场', !!genBtn());
...
const DRAFT_TEXT = '速度场(mintpy/velocity.h5)显示:LOS 形变速率介于 -40.0 至 20.0 mm/yr……'
  + '交叉验证相关系数 crossval_r = 0.92,达到台账阈值 0.85。';
fetchScript = [{ status: 200, payload: {
  run_id: '20260813T110000-check01', draft: DRAFT_TEXT,
  llm_polish: true, facts_used: ['velocity.min_mmyr=-40.0'], saved: true,
} }];
genBtn().click();
await sleep(10);
check('POST /api/report/results 且 body 带 session',
  fetchCalls.length === 1 && fetchCalls[0].url === '/api/report/results'
...
check('落盘注明 report_results.md',
  mounted()[0].textContent.includes('report_results.md'));
check('run 短标识在场', mounted()[0].textContent.includes('20260813T110000'));

/* ---------------- ④ 一键复制 ---------------- */
console.log('\n== ④ 一键复制 ==');
const copyBtn = mounted()[0].querySelectorAll('button')
...
check('toast 复制文案:已复制 + 可直接改写进论文',
  toasts.some((t) => t.textContent.includes('已复制结果章节草稿')
    && t.textContent.includes('可直接改写进论文')));

/* ---------------- ⑤ 骨架回退徽章 ---------------- */
console.log('\n== ⑤ 骨架回退 ==');
fetchScript = [{ status: 200, payload: {
  run_id: '20260813T110000-check01', draft: '骨架文本:QA 总体判定为 pass。',
...
genBtn().click();
await sleep(10);
check('404 → 「还没有可生成结果章节的 run」提示',
  mounted()[0].textContent.includes('还没有可生成结果章节的 run'));
