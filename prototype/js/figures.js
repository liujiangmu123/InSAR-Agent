...
/* ---------------- 时序折线图 ----------------
   series 可选扩展(向后兼容,tspoint.js 真实曲线用):
...
/** 右侧小地图：形变底图 + 可点点位。
    opts 扩展(向后兼容,tspoint.js 真实模式用):
      hidePoints —— 隐藏演示点位(真实模式点击任意位置取像元,演示点名无意义);
      markers    —— 已选真实点标记 [{x, y, color, label}](viewBox 300×170 坐标);
      note       —— 左下角小注(如真实数据的覆盖范围/网格说明)。 */
export function mapSvg(selectedId, opts = {}) {
  const uid = ++uidSeq;
  const pts = opts.hidePoints ? '' : POINTS.map((p) => {
