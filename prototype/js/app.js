...
function boot() {
  ...
  St.initSteps();   // 镜像启动为空:计划只来自服务端 /api/state
...
/* ============================================================
   服务端水合:GET /api/sessions（列表）→ GET /api/registry（步骤目录）
   → GET /api/state（步骤镜像）+ GET /api/chat（历史重放）
   + /api/events 全局事件通道（断线自动重连，见 backend.sse.js）
   ============================================================ */

/** 会话列表（含已归档,供折叠组）。网络失败/非 2xx → null=后端不可达。 */
async function fetchSessionsList() {
  if (location.protocol === 'file:') return null;
  try {
    const resp = await fetch('/api/sessions?include_archived=1');
...
async function ensureRegistry() {
  ...
  try {
    ...
    if (Array.isArray(caps) && caps.length) St.setRegistry(caps);
...
async function hydrateFromServer() {
  const rows = await fetchSessionsList();
  ...
  Dock.refresh();
  if (!S.sessionId) return;   // 零会话:聊天区保持引导空态,等待「新建会话」
  ...
  connectGlobalEvents();   // SSE 只在确有会话时接通（端点会为未知会话建目录）
}
...
function connectGlobalEvents() {
  dropEvents?.();
  // 无会话不开 SSE：/api/events 会为未知会话在服务端建目录
  dropEvents = S.sessionId ? API.connectEvents(onGlobalEvent) : null;
}
...
function renderHero() {
  Stream.renderHero((p) => { el.input.value = p; submit(); });   // 引导空态:示例只填充不发送
}
...
function switchSession(s) {
  ...
  paintSubtitle();
  reset();
  hydrateFromServer();      // 新会话的服务端状态与历史（含 SSE 重挂）
  toast(`已切换会话：${s.name}`);
}

/**
 * 新建会话:POST /api/sessions 真实创建,成功后本地入列并切换。
...
async function createSession() {
  ...
  try {
    ...
    if (!row) {
      paintOffline(true);
      toast('后端不可达，无法新建会话——请先启动本机服务', 4200);
      return null;
    }
    paintOffline(false);
...
function paintStatus() {
  ...
  const running = steps.find((st) => st.state === 'running');
  // §7.8：interrupted / orphaned / failed 都属「可从断点处置续跑」
  const resumable = steps.some((st) =>
    ['interrupted', 'orphaned', 'failed'].includes(st.state));

  let kind = '', text = '空闲';
  if (S.busy && running) { kind = 'run'; text = `运行中 · 第 ${running.id}/${total} 步`; }
  ...
  else if (pending.length) { kind = 'run'; text = `${pending.length} 步待运行`; }
  else if (total && !all.length) { kind = 'ok'; text = '全部完成 · 指纹一致'; }
  // total===0（无计划）保持「空闲」——不再假装存在一条已完成的流水线
...
/* ============================================================
   顶栏磁盘预算条（§7.5 三级规则）：
     未知/> 20G 不显示；8–20G 灰字「磁盘 XXG↓」；< 8G 橙色 ⚠
   数值只来自服务端 budget 事件 —— 没有事件就不显示,不编数。
   ============================================================ */
function fmtGB(gb) {
...
function paintBudget() {
  ...
  if (gb == null || !Number.isFinite(gb) || gb > DISK_TIERS.hide) { box.hidden = true; return; }
  box.hidden = false;
  if (gb >= DISK_TIERS.warn) {
    box.className = 'budget';
    box.title = `工作区磁盘剩余 ${fmtGB(gb)} · 低于 ${DISK_TIERS.hide}G 后常态显示`;
    box.replaceChildren(txt(`磁盘 ${fmtGB(gb)}↓`));
  } else {
    box.className = 'budget is-warn';
    box.title = `工作区磁盘剩余 ${fmtGB(gb)} · 低于 ${DISK_TIERS.warn}G 触发告警（§4.10 三级闸门）`;
    box.replaceChildren(txt(`⚠ 磁盘 ${fmtGB(gb)}`));
  }
}
...
async function consume(iter) {
  for await (const ev of iter) {
    switch (ev.t) {
      ...
      case 'candidates':
        // 注册表未加载（离线启动）时无候选目录可渲染,跳过而非半渲染崩溃
        if (!def_(ev.stepId)) break;
      ...
      /* 结果/报告收尾:不再渲染写死数值的演示卡片 —— 真实产物在右侧
         「文件/影像/报告」面板由各自的 /api 数据源呈现,这里只做导航。 */
      case 'result':
        currentPlan?.set(4, 'd');
        currentPlan?.set(5, 'd');
        ...
        Dock.refresh();
        Stream.note('ok', '运行结束——产物与图件已就绪，见右侧「文件」「影像」面板。', [
          { label: '查看产物', run: () => { Dock.setTab('files'); ensureDock(); } },
          { label: '查看图件', run: () => { Dock.openImages(); ensureDock(); } },
        ]);
        break;

      case 'report':
        currentPlan?.set(6, 'd');
        Stream.note('info', '方法草稿与 provenance 已生成，可在右侧「报告」「审计」面板查看。', [
          { label: '打开报告面板', run: () => { Dock.setTab('report'); ensureDock(); } },
        ]);
        break;
    }
  }
...
/* ============================================================
   附件（仅名称）：本地选文件 → 输入框上方文件名 chip，可删除；
   不读取内容、不上传，发送时仅把文件名并入消息文本。
   ============================================================ */
function wireAttach() {
  const input = $('#fileInput');
  $('#btnAttach').addEventListener('click', () => {
    toast('附件暂不上传内容，仅以文件名 chip 附在消息上');
    input.click();
  });
...
async function submit() {
  ...
  // 还没有会话就直接输入 → 先真实创建一个（与「新建会话」同一 API 通路）
  if (!S.sessionId && !(await createSession())) return;
  if (attachments.length) {
    text += `\n〔附件 · 未上传，仅名称〕${attachments.map((a) => a.name).join('、')}`;
...
function abort() {
  ...
  let hit = false;
  S.steps.forEach((st) => {
    if (st.state === 'running') { hit = true; St.setStepState(st.id, 'interrupted', { stale: true }); }
  });
...
function resumeRun() {
  ...
  const resumable = [...S.steps.values()].some((st) =>
    ['interrupted', 'orphaned', 'failed'].includes(st.state));
...
function applyMethod(stepId, methodId) {
  if (!st_(stepId)) { toast('该步骤尚无执行计划——先对话生成计划再改方法'); return; }
...
function applyParams(stepId, patch) {
  if (!st_(stepId)) { toast('该步骤尚无执行计划——先对话生成计划再改参数'); return; }
  const snap = St.snapshotSteps();
...
function interventionDuringRun(what) {
  ...
  const running = [...S.steps.values()].find((st) => st.state === 'running');
  Stream.interventionEntry({
    mode: 'queue',
    text: `你在第 ${running?.id ?? '?'} 步运行中${what} → 已排入队列，当前步完成后生效`,
  });
  Dock.refresh();
  paintStatus();
  return true;
...
function explainInvalidation(stepId, from, to, affected, kind, snap = null) {
  const name = def_(stepId)?.name || `步骤 ${stepId}`;
  const staleIds = affected.filter((id) => st_(id)?.stale);
  Dock.flashSteps(affected);
  Dock.refresh();

  Stream.agentMsg(
    `已更新第 ${stepId} 步（${name}）的${kind === 'method' ? '方法' : '参数'}：`,
  ...
  // 不再渲染 provenanceCard:其工具/环境/git 行是写死的演示值;
  // 真实证据链由服务端 /api/provenance 在「审计」面板呈现。
...
function askRerun() {
  ...
  const card = Stream.askApproval({
    ...
    actions: [
      ...
      // 首个待跑步骤有方法目录时才给「改用其它方法」——不再写死演示决策点
      ...(def_(sum.ids[0]) ? [{
        label: '改用其它方法', done: '已取消 · 返回候选集',
        run: () => Stream.candidateSet({
          stepId: sum.ids[0], onPick: (m) => applyMethod(sum.ids[0], m),
        }),
      }] : []),
...
async function run(ids) {
  ...
  try {
    ...
    // 失败/中断停链（§4.12 专家模式必须问）——此时是暂停不是完成
    const broken = [...S.steps.values()].some((st) =>
      ['failed', 'interrupted', 'orphaned'].includes(st.state));
...
/** 有计划的步骤镜像（升序）。导出只反映真实存在的计划,没有就是空表。 */
function plannedSteps() {
  return [...S.steps.values()].sort((a, b) => a.id - b.id);
}

/**
 * 方法章节草稿（客户端快照）：只导出当前会话真实的方法/参数/指纹表。
 * 不含任何编造的数值结果——真实方法草稿以服务端 /api/methods.md 为准
 * （右侧「报告」面板），此导出仅作离线快照。
 */
function buildMethodsMarkdown() {
  ...
  const rows = steps.map((st) => {
    ...
    return `| ${st.id} | ${def_(st.id)?.name || `步骤 ${st.id}`} | \`${st.method}\` | ${fmtParams(st.params)} | \`${st.fingerprint}\` | ${state} |`;
  });
  return `# 方法章节草稿（客户端状态快照）

> 由 InSAR-Agent 界面于 ${new Date().toISOString()} 生成，会话 ${S.sessionId ?? '—'}。
> 内容仅为当前会话的处理链配置快照；**完整方法叙述与数值结果以服务端
> 生成的 methods_draft.md（报告面板）为准**，本文件不含任何结果数值。

## 处理链快照 · ${steps.length} 步
...
${S.degraded.map((g) => `- 降级记录：第 ${g.stepId} 步 \`${g.from}\` → \`${g.to}\`（${g.failClass}），证据代价已入账（§4.12 降级矩阵）`).join('\n')}${S.degraded.length ? '\n' : ''}- 本快照只描述配置与状态，不声称任何形变量值
`;
}

/** provenance 快照：客户端会话状态,非服务端账本（账本见「审计」面板）。 */
function buildProvenance() {
  return {
    schema_version: '1.0',
    generated_at: new Date().toISOString(),
    session: S.sessionId,
    note: '客户端会话状态快照：指纹为前端 FNV-1a 摘要（非 sha256）；权威证据链以服务端 /api/provenance 为准。',
    mode: S.mode,
    phase: S.phase,
    ...
    steps: plannedSteps().map((st) => ({
      id: st.id, name: def_(st.id)?.name || `步骤 ${st.id}`, deps: def_(st.id)?.deps || [],
      method: st.method, params: st.params,
      fingerprint: st.fingerprint, state: st.state, stale: st.stale,
      command: Dock.buildCmd(st.id).replace(/^\$ /, ''),
      outputs: (def_(st.id)?.outputs || []).map((o) => o.path),
    })),
    environment: { runtime: 'browser-snapshot' },
  };
}
...
function buildRunScript() {
  ...
  const body = steps.map((st) => {
    return `# [${String(st.id).padStart(2, '0')}/${steps.length}] ${def_(st.id)?.name || `步骤 ${st.id}`} · fingerprint ${st.fingerprint}\n`
      + Dock.buildCmd(st.id).replace(/^\$ /, '');
  }).join('\n\n');
  return `#!/usr/bin/env bash
# InSAR-Agent 等价裸命令脚本 · 会话 ${S.sessionId ?? '—'}
# 客户端快照：命令由当前 method/params 动态生成，可脱离 Agent 审阅复现路径。
set -euo pipefail
...
function doExport(kind) {
  if (!S.steps.size && kind !== 'json') { toast('还没有执行计划——对话或运行流水线后再导出'); return; }
  if (kind === 'md') {
    download('methods_draft.md', buildMethodsMarkdown(), 'text/markdown');
    toast('已导出 methods_draft.md · 客户端状态快照');
  } else if (kind === 'json') {
    ...
    toast('已导出 run.sh · 等价裸命令');
  } else {
    toast('不提供 ZIP 打包（零依赖），请分别导出 .md / .json / .sh');
  }
}
...
function reset() {
  expireUndos();
  St.initSteps();                // 镜像清空:计划由目标会话的 /api/state 重新水合
  S.phase = 'idle';
  S.selectedStep = null;
  S.selectedFile = null;
  S.degraded.length = 0;
  ...
  lastChange = null;
  S.evidenceLevel = St.evidenceCeiling().level;
  St.setDiskFree(null);          // 预算未知:只信服务端 budget 事件
  currentPlan = null;
  tools.clear();
  attachments.length = 0;
